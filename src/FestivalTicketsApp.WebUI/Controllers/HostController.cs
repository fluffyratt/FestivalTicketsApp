...
public class HostController(IHostService service) : Controller
{
    ...
    public async Task<IActionResult> List(int id, [FromQuery, Bind(Prefix = "QueryState")] HostListQuery query)
    {
        ...
        Result<List<string>> getCityNamesResult = await _service.GetCitiesAsync();
        ...
        Result<Paginated<HostDto>> getHostsResult = await _service.GetHostsAsync(hostFilter);
    ...
    public async Task<IActionResult> Details(int id)    
    {
        ...
        var getHostResult = await _service.GetHostWithDetailsAsync(hostId);
        ...
        viewModel.HostedEvents = (await _service.GetHostedEventsAsync(hostId)).Value;
