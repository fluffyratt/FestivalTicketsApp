...
public class TicketService(AppDbContext context) : ITicketService
{
    ...
    public async Task<Result<List<TicketDto>>> GetEventTicketsAsync(int eventId)
    {
    ...
    public async Task<Result<List<TicketWithPriceDto>>> GetTicketsWithPriceByIdAsync(List<int> ticketsId)
    {
        IQueryable<Ticket> ticketsQuery = _context.Tickets
            .AsNoTracking()
            .Include(t => t.TicketType);
    ...
    public async Task<Result<List<TicketTypeDto>>> GetEventTicketTypesAsync(int eventId)
    {
        bool isEventExist = await _context.Events.AnyAsync(e => e.Id == eventId);
    ...
    public async Task<Result> ChangeEventTicketsStatusAsync(string statusName, List<int> ticketsId, int? clientId)
    {
        IQueryable<Ticket> ticketsQuery = _context.Tickets
            .Include(t => t.TicketStatus);
        ...
        {
            Result? intermediateResult = await ProcessStatusChange(ticketEntity, needStatus, clientId);
    ...
    private Task<Result?> ProcessStatusChange(Ticket ticketEntity, TicketStatus nextStatus, int? clientId)
    {
        if (ticketEntity.TicketStatus.Status == ServicesEnums.TicketPurchasedStatus 
          ...
          && clientId is not null)
            return Task.FromResult<Result?>(Result.Failure(DomainErrors.SoldTicketCantBeSoldAgain));
        
        if (nextStatus.Status == ServicesEnums.TicketPurchasedStatus 
         && clientId is null)
            return Task.FromResult<Result?>(Result.Failure(DomainErrors.AnonymousTicketBuy));

        if (nextStatus.Status == ServicesEnums.TicketOutOfDateStatus)
            return Task.FromResult<Result?>(Result.Failure(DomainErrors.OutOfDateTicketStatusCantBeChanged));

        if (ticketEntity.TicketStatus.Status == nextStatus.Status)
            return Task.FromResult<Result?>(Result.Failure(DomainErrors.SameTicketStatusSet));
